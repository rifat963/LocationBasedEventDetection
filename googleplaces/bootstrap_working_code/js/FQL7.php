...
$place_search2 = file_get_contents('https://graph.facebook.com/search?type=place&center=' . $lat . ',' . $long . '&distance=500&access_token=' . $fb_token);
//echo $place_search2;
...
foreach($place_search->data as $result) {
   ...
   // $fql = 'SELECT checkin_count FROM place WHERE page_id=' .$result->id. '';
    ...
    foreach ($result2->data2 as $value) {
        echo 'checkin_count="' .$value->checkin_count. '" ';  
    }
