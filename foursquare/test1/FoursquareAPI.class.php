...
class FoursquareApi {
	...
	public $BaseUrl = "https://api.foursquare.com/";
	...
	public $AuthorizeUrl = "https://foursquare.com/oauth2/authorize";
	...
	public function GetPublic($endpoint,$params=false){
		...
		// Return the result;
                echo $this->GET($url,$params);
		return $this->GET($url,$params);
               
               // echo $this->Request($url,$params,HTTP_GET);
		//return $this->Request($url,$params,HTTP_GET);
	}
        public function poiGET($endpoint,$params=false){
               // echo $this->Request($url,$params,HTTP_GET);
            // Build the endpoint URL
		$url = $this->BaseUrl . trim($endpoint,"/");
		...
		$params['v'] = $this->Version;
		$params['locale'] = $this->ClientLanguage;
                $test= $this->Request($url,$params,HTTP_GET);
                echo $test;
		return $this->Request($url,$params,HTTP_GET);
	}
        
        public function getValue($endpoint,$params=false)
        {
            // Build the endpoint URL
		$url = $this->BaseUrl . trim($endpoint,"/");
	 ...
	 */
	public function Request($url, $params=false, $type=HTTP_GET){
        ...
        public function getPOI($url, $params=false, $type=HTTP_GET){
	...
	public function GET($url,$params=false){
            $value= $this->Request($url,$params,HTTP_GET);
               // echo $value;
            $username="root";
            $password="";
            ...
            $db_selected = mysql_select_db($database, $connection);
             $query1 = "DELETE FROM poi_table";
            $result = mysql_query($query1);
            
            $query = sprintf("INSERT INTO poi_table " .
                 " ( poi_data ) " .
                  " VALUES ('%s');",
